#pragma once
// ─── FindCallersTool ──────────────────────────────────────────────────────────
// Analysis: Welche Funktionen rufen diese Funktion auf?

#include "../ClangToolBase.h"

class FindCallersTool : public ClangToolBase
{
public:
    using ClangToolBase::ClangToolBase;

    QString name() const override { return "find_callers"; }

    QString description() const override
    {
        return "Find all functions that call the specified function. "
               "Shows who calls whom (inverse call graph).";
    }

    QJsonObject properties() const override
    {
        return {
            {"file", prop("string", "File to analyze")},
            {"function_name", prop("string", "Function name to find callers for")}
        };
    }

    QJsonArray required() const override { return {"file", "function_name"}; }

    ToolResult execute(const QJsonObject &args) override
    {
        QString file = args.value("file").toString();
        QString funcName = args.value("function_name").toString();

        if (file.isEmpty() || funcName.isEmpty())
            return ToolResult::err("Error: 'file' and 'function_name' are required.");

        if (!parseFile(file))
            return ToolResult::err("Error: Could not parse file: " + m_clang->diagnostics().join("\n"));

        QString usr;
        auto funcs = m_clang->getAllFunctions();
        for (const auto &f : funcs) {
            if (f.name == funcName || f.displayName == funcName) {
                usr = f.usr;
                break;
            }
        }

        if (usr.isEmpty())
            return ToolResult::err(QString("Function '%1' not found.").arg(funcName));

        auto callers = m_clang->getCallers(usr);
        
        if (callers.isEmpty())
            return ToolResult::ok(QString("No callers found for '%1'.").arg(funcName));

        return ToolResult::ok(formatSymbols(callers, QString("Callers of %1").arg(funcName)));
    }
};