...
QString AgentPlan::buildPlannerSystemPrompt(const QString &auftrag) const
{
    ...
    return QString(
        ...
        "    \"title\":       \"Timer::start() implementieren\",\n"
        "    \"level\":       3,\n"
        "    \"scope\":       \"internal\",\n"
        "    \"description\": \"Startet den QTimer, setzt m_running=true\",\n"
        "    \"symbol\":      \"Timer::start()\",\n"
        "    \"parent_id\":   5\n"
        "  }}</tool_call>\n"
        ...
        "level 2 (H2) — Einzelne Datei (title = Dateiname, z.B. 'timer.cpp')\n"
        "level 3 (H3) — Eine Methode oder Funktion innerhalb der Datei\n"
        "\n"
        "H3-REGEL:\n"
        "  .h Dateien      → H2-Blatt (kein H3 noetig, Interface in einem Stueck)\n"
        "  .cpp Dateien    → H2 + H3-Kinder (jede Methode ein eigener Node)\n"
        "  CMakeLists.txt  → H2-Blatt\n"
        "\n"
        "H3-TITEL-REGEL (sehr wichtig!):\n"
        "  Der title bei H3 = Methodenname oder kurze Aufgabe\n"
        "  NICHT der Dateiname — der Dateiname steht schon beim H2-Elterknoten!\n"
        "\n"
        "  RICHTIG:  title = \"Timer::start() implementieren\"\n"
        "  RICHTIG:  title = \"start()\"\n"
        "  FALSCH:   title = \"src/timer.cpp\"  ← das ist der H2-Titel!\n"
        "\n"
        "  Der H3-title muss zeigen WAS implementiert wird, nicht WO.\n"
        "\n"
        ...
        "  Beispiele fuer das symbol-Feld:\n"
        "    H2 .h:   'Timer'              (Klassen-Interface)\n"
        "    H2 .cpp: 'Timer'              (Datei insgesamt)\n"
        "    H3:      'Timer::start()'     (Methode)\n"
        "    H3:      'Timer::m_interval'  (Member)\n"
        "\n"
        "── BEISPIEL (korrekter Aufbau) ───────────────────────────────────────\n"
        "\n"
        "Fuer eine Datei 'timer.cpp' mit den Methoden start() und stop():\n"
        "\n"
        "  H2-Node: title='timer.cpp', symbol='Timer', level=2\n"
        "    H3-Node: title='Timer::start() implementieren', symbol='Timer::start()', level=3\n"
        "    H3-Node: title='Timer::stop() implementieren',  symbol='Timer::stop()',  level=3\n"
        "\n"
        "── REIHENFOLGE ───────────────────────────────────────────────────────\n"
        "1. Erst Lese-Tools um Projekt zu verstehen (optional)\n"
        "2. H0 anlegen (Wurzel, parent_id=-1)\n"
        "3. H1-Gruppen anlegen (optional, fuer groessere Projekte)\n"
        "4. H2-Dateien anlegen (title = Dateiname)\n"
        "5. H3-Methoden anlegen (title = Methodenname, NICHT Dateiname)\n"
